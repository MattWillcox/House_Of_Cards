$(() => {
  ...
  function cardPlay(index) { // socket connection
...
socket.on('gameOver', function(state){
  ...
  socket.on('load', function(state) {  // Receives the data ; allows parsing of the chosen card
    var frontClasses = ['ace', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'jack', 'queen', 'king'];
    var state = JSON.parse(state);
    $('.card.stack').empty();
    $('<div>').addClass('card spades clickable ' + frontClasses[state.prizeCard - 1])
  ...
  socket.on('updatePrizeCard' , function(state){ // enables update of each respective players score
    console.log("in updatePrizeCard");
    var frontClasses = ['ace', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten', 'jack', 'queen', 'king'];
    var state = JSON.parse(state);
    $('.card.stack').empty();
  ...
  socket.on('gameOver', function(state){ // Enables Gameover conditions
    var state = JSON.parse(state);
    $('.scoreboard').empty();
    if (state.p1Score > state.p2Score){
      $('<div>').addClass('p1Winner').text("P1 WINS CONGRATS")
  ...
  socket.on('result', function(state){ // Result is sent to the databse
  ...
  console.log('outside', playerNum); // rendering of each respective players cards
