module.exports = function(io, knex) {
  ...
  var preShuffleState = {  // This is a state where both players are at before initiation of the game
  ...
  function shuffleCards(state) { // Tis is a state where "cards" get shuffled and randomized
...
function shuffleCards(state) {
  ...
  let prizeCard = deck.pop(); // Enables removal of a "card" after every turn.
  const nextState = {
    gameState: 'SHUFFLED',
    p1Hand: state.p1Hand,
    p2Hand: state.p2Hand,
  ...
  function revealPrize(state) { // This is state where each "card" becomes revealed
  ...
  function compareCards(state, card1, card2) { // This is state where p1 and p2 compares.
    const winner = Number(card1) - Number(card2); // Once winner is defined, scores are updated depending on the winner.
    let nextState = {};
    if(winner > 0) {
