...
function shuffleCards(state) {
  ...
  const initialState = shuffleCards(preShuffleState);
  ...
  io.on('connection', function(client) {
    ...
    client.on('cardplay', function(data) {
      const move = JSON.parse(data);
        ...
        if(playerNumber === 1){
          console.log('differece', countNulls(initialState.p1Hand, initialState.p2Hand));
          if(countNulls(initialState.p1Hand, initialState.p2Hand) <= 0){
            console.log("p wait please.");
              ...
              initialState['p' + playerNumber + 'Hand'][move.index] = null;
