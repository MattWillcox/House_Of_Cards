...
function shuffleCards(state) {
  ...
  var goofspiel = io.of('/goofspiel').on('connection', function(client) {
    ...
    client.on('cardplay', function(data) {
      ...
      const move = JSON.parse(data);
        ...
        if(playerNumber === 1){
          if(countNulls(initialState.p1Hand, initialState.p2Hand) <= 0){
            console.log("p wait please.");
              ...
              goofspiel.emit('yourTurn', " It is player 2's turn");
          }
        } if(playerNumber === 2) {
            if(countNulls(initialState.p1Hand, initialState.p2Hand) <= 0){
              ...
              console.log("new state:", initialState);
              client.emit('update state', JSON.stringify(initialState));
              goofspiel.emit('yourTurn', " It is player 1's turn");
        ...
        if(countNulls(initialState.p1Hand, initialState.p2Hand) == 0){
          ...
          goofspiel.emit('yourTurn', " It's both of your turn" )

          if(!initialState.prizeCard){
            console.log('GAME OVER');
            goofspiel.emit("gameOver", JSON.stringify(initialState))
            let result = initialState.p1Score - initialState.p2Score;
            if(result > 0)
              client.emit("result", 1);
            if(result < 0)
              client.emit('result', 2);
            if(result === 0)
              client.emit('result', 0);
          } else {
            goofspiel.emit("updatePrizeCard", JSON.stringify(initialState))
          }
