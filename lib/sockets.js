  ...
  socket.on('cardplay', function(data) {
    ...
    if(playerNumber === 1){
      ...
      if(countNulls(initialState.p1Hand, initialState.p2Hand) >= 0){
          ...
          goofspiel.to('room 1').emit('yourTurn', " It is player 2's turn");
      }
    }
    if(playerNumber === 2) {
      if(countNulls(initialState.p1Hand, initialState.p2Hand) <= 0){
        ...
        socket.emit('update state', JSON.stringify(initialState));
        goofspiel.to('room 1').emit('yourTurn', " It is player 1's turn");
      }
      if(countNulls(initialState.p1Hand, initialState.p2Hand) >= 0){
      }
    }
    if(countNulls(initialState.p1Hand, initialState.p2Hand) == 0){
      ...
      goofspiel.to('room 1').emit('yourTurn', "Make a move");

      if(!initialState.prizeCard){
       goofspiel.to('room 1').emit("gameOver", JSON.stringify(initialState))
        ...
        goofspiel.to('room 1').emit("updatePrizeCard", JSON.stringify(initialState))
      }
    }
  })
});
